#!/usr/bin/env python
"""
Fix Tamim's form level assignment and show fees
"""
...
def fix_tamim_form():
    """Fix Tamim's form level assignment and show fees"""
    print("=" * 60)
    print("FIXING TAMIM'S FORM LEVEL ASSIGNMENT")
    print("=" * 60)
    ...
    # Check if we need to assign to Form 3
    if student.level == 'form' and student.level_custom == 'Form 3':
        print(f"\n✅ Tamim is already assigned to Form 3!")
    else:
        print(f"\n🔧 ASSIGNING TAMIM TO FORM 3...")
        
        # Update student to Form 3
        student.level = 'form'
        student.level_custom = 'Form 3'
        student.save()
        
        print(f"   ✅ Updated {student.first_name} to Form 3")
        print(f"   New Level: {student.level}")
        print(f"   New Level Custom: {student.level_custom}")
        print(f"   New Display Value: {student.get_level_display_value()}")
    
    # Now check what fees Form 3 students should see
